...
class Embedder:
    ...
    def __init__(self, **kwargs):
        ...
        self.kwargs = kwargs
        if self.kwargs['interpolate_low_freq']:
            self.create_simplified_embedding_fn()
        else:
    ...
    def create_simplified_embedding_fn(self):
        ...
        for freq in freq_bands:
            if freq <= (freq // 3):
                print("LOOK HERE SOMETHING WORKED")
            else:
                for p_fn in self.kwargs['periodic_fns']:
                    embed_fns.append(lambda x, p_fn=p_fn,
                                     freq=freq: p_fn(x * freq))
...
def get_embedder(multires, i=0, int_low_freq=False):

    if i == -1:
        return tf.identity, 3

    embed_kwargs = {
        'interpolate_low_freq': int_low_freq,
        'include_input': True,
        'input_dims': 3,
